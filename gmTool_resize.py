...
class GMToolWindow(tk.Toplevel):
    ...
    def buildTab_server(self,tabView:ttk.Notebook,tabName:str):
        ...
        if True:
            ...
            ttk.Button(normalFuncFrame,text='启动服务器',command=run_server,width=int(WIDTH*14)).grid(row=3,column=2,**cfg)
            ttk.Button(normalFuncFrame,text='停止服务器',command=stop_server,width=int(WIDTH*14)).grid(row=3,column=3,**cfg)
            ttk.Button(normalFuncFrame,text='重启频道',command=restart_channel,width=int(WIDTH*14)).grid(row=3,column=4,**cfg)
