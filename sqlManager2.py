...
def execute_and_fech(db,sql,args=None,charset='utf8'):
    taskID = datetime.datetime.now().strftime('%Y%m%d%H%M%S%f')
...
def getCharactorInfo(cName='',uid=0):
    ...
    if uid>=0 and cName=='':
        ...
        res = list(execute_and_fech('taiwan_cain',sql,(name_new,)))

        name_tw = convert(cName,'zh-tw')
        if cName!=name_tw:
            name_tw_new = name_tw.encode('utf-8','replace')
            sql = f"select m_id, charac_no, charac_name, lev, job, grow_type, delete_flag, expert_job from charac_info where charac_name=%s;"
            res_tmp = execute_and_fech('taiwan_cain',sql,(name_tw_new,))
            res.extend(res_tmp)
...
def connect(infoFunc=lambda x:...,conn=None): #多线程连接
    ...
    def innerThread():
        for i,connector_ in enumerate(SQL_CONNECTOR_LIST):
            try:
                db = connector_.connect(user=config['DB_USER'], password=config['DB_PWD'], host=config['DB_IP'], port=config['DB_PORT'], database='d_taiwan',charset='utf8',**SQL_CONNECTOR_IIMEOUT_KW_LIST[i])
